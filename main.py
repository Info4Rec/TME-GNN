...
parser.add_argument('--in_rate', type=float, default=0.05)
...
parser.add_argument('--sslrate', type=float, default=0.01, help='SSL rate.')
...
def main():
    ...
    datato = open("test1.txt",'a+')
    print(opt,file=datato)
    datato.close()
    start = time.time()
    best_result = [0, 0]
    best_epoch = [0, 0]
    ...
    for epoch in range(opt.epoch):
        datato = open("test1.txt", 'a+')
        print('-------------------------------------------------------', file=datato)
        print('epoch: ', epoch, file=datato)
        datato.close()
        model.epoch = epoch
        hit_20, mrr_20= train_test(model, train_data, test_data, opt.sslrate)
        flag = 0
        ...
        if bad_counter >= opt.patience:
            break
        # state = {'Eiters': model.Eiters, 'epoch': model.epoch, 'model': model.state_dict(),
